...
project_path <- "/home/vaibhav11/GSEA1/GSK591_vs_DMSO_6d"
in_path <- file.path(project_path)
out_path <- file.path(project_path, "GSEA_result/C2CP")  
#msigdb data input
gene_sets_df <- msigdbr(species = "Homo sapiens", collection = "C2",subcollection ="CP" )
cat(paste("loaded", length(gene_sets_df), "gene sets\n"))

#df
gene_sets <- gene_sets_df %>% split(x = .$gene_symbol, f = .$gs_name)
df <- read.delim(file.path(in_path, 'diff-exp-fdr-0.01.txt'), row.names = 1)
...
write.table(gsea_res, file = file.path(out_path, 'gsea_c2CP.txt'), sep = "\t", quote = F, row.names = F)
...
write.table(toppathwaysDN, file=file.path(out_path,"top10DN.txt"),sep = "\t",quote = F,row.names = F)
write.table(toppathwaysUP, file=file.path(out_path,"top10UP.txt"),sep = "\t",quote = F,row.names = F)

topPathways <- c(toppathwaysUP, rev(toppathwaysDN))

# gseaplot
pdf(file = file.path(out_path, paste0("c2CP_", 'pathway.pdf')), width = 20, height = 15)
...
ggsave(file = file.path(out_path, "barplot_toppathwaysUP.pdf"), width = 30, height = 10)
...
ggsave(file = file.path(out_path, "dotplot_toppathwaysUP.pdf"), width = 10, height = 6)

# Cnet
up_gene_sets <- gene_sets[toppathwaysUP]
pdf(file = file.path(out_path, "cnetplot_upregulated.pdf"), width = 30, height = 30)
...
pdf(file = file.path(out_path, "cnetplot_downregulated.pdf"), width = 35, height = 30)
