...
out_path <- file.path(project_path, "GSEA_result/C5")  
#msigdb data input
gene_sets_df <- msigdbr(species = "Homo sapiens", collection = "C5")
...
write.table(gsea_res, file = file.path(out_path, 'gsea_c5.txt'), sep = "\t", quote = F, row.names = F)
...
# gseaplot
pdf(file = file.path(out_path, paste0("c5_", 'pathway.pdf')), width = 20, height = 15)
...
ggsave(file = file.path(out_path, "barplot_toppathwaysUP.pdf"), width = 10, height = 6)
